  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 ease-out max-w-5xl mx-auto">
      <header className="flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 mb-2">Social Connections</h1>
          <p className="text-slate-500 text-sm">Connect your social media accounts to enable auto-posting.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {PLATFORMS.map((platform) => {
          ...
          const buttonClass = `px-4 py-2 rounded-xl text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#0a0a0a] ${
            isConnected 
            ? "bg-white text-slate-700 hover:bg-red-500/10 hover:text-red-600 hover:border-red-500/50 border border-slate-200" 
